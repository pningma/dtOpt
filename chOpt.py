...
prob1 += pl.lpSum([ch_stat['模型通过率'][i] * p[i]
                  for i in rng]) >= MIN_APPROVAL_RATE
prob1 += p[1] >= 0.2
prob1 += p[2] >= 0.2
prob1 += p[3] >= 0.35
prob1 += pl.lpSum([ch_stat['损失率'][i] * p[i] for i in rng]) <= MAX_LOSS_RATE
prob1 += pl.lpSum([ch_stat['加权利率'][i] * p[i]
                  for i in rng]) >= MIN_WEIGHTED_INTEREST_RATE
# 求解
prob1.solve(solver)
...
print(p_opt1)
print('[约束] 总体通过率: ', np.round(sum(ch_stat['模型通过率'][i] * list(p_opt1.values())[i]
      for i in rng), 3), ' (', MIN_APPROVAL_RATE, ')', sep='')
print('[约束] 总体损失率: ', np.round(sum(ch_stat['损失率'][i] * list(p_opt1.values())[i]
      for i in rng), 3), ' (', MAX_LOSS_RATE, ')', sep='')
print('[约束] 总体加权利率: ', np.round(sum(ch_stat['加权利率'][i] * list(p_opt1.values())[i]
      for i in rng), 3), ' (', MIN_WEIGHTED_INTEREST_RATE, ')', sep='')
print('[目标] FTP扣除前利润: ', np.round(sum(ch_stat['FTP扣除前利润'][i]
      * list(p_opt1.values())[i] for i in rng), 2), sep='')
...
prob2 += pl.lpSum([ch_stat['模型通过率'][i] * p[i]
                  for i in rng]) >= MIN_APPROVAL_RATE
prob2 += p[1] >= 0.2
prob2 += p[2] >= 0.2
prob2 += p[3] >= 0.35
prob2 += pl.lpSum([ch_stat['损失率'][i] * p[i] for i in rng]) <= MAX_LOSS_RATE
prob2 += pl.lpSum([ch_stat['加权利率'][i] * p[i]
                  for i in rng]) >= MIN_WEIGHTED_INTEREST_RATE
prob2 += pl.lpSum([ch_stat['FTP扣除前利润率'][i] * p[i]
                  for i in rng]) >= MIN_PROFIT_RATE
# 求解
prob2.solve(solver)
...
print(p_opt2)
print('[约束] 总体通过率: ', np.round(sum(ch_stat['模型通过率'][i] * list(p_opt2.values())[i]
      for i in rng), 3), ' (', MIN_APPROVAL_RATE, ')', sep='')
print('[约束] 总体损失率: ', np.round(sum(ch_stat['损失率'][i] * list(p_opt2.values())[i]
      for i in rng), 3), ' (', MAX_LOSS_RATE, ')', sep='')
print('[约束] 总体加权利率: ', np.round(sum(ch_stat['加权利率'][i] * list(p_opt2.values())[i]
      for i in rng), 3), ' (', MIN_WEIGHTED_INTEREST_RATE, ')', sep='')
print('[约束] FTP扣除前利率: ', np.round(sum(ch_stat['FTP扣除前利润率'][i] * list(p_opt2.values())[i]
      for i in rng), 3), ' (', MIN_PROFIT_RATE, ')', sep='')
print('[目标] 放款金额: ', np.round(sum(ch_stat['放款金额'][i] *
      list(p_opt2.values())[i] for i in rng), 2), sep='')
